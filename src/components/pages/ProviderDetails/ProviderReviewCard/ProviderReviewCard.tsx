...
const ProviderReviewCard: React.FC<ServiceCardProps> = ({ providerId }) => {
  const { data } = useReviewByProviderIdQuery(providerId);
  ...
  if (!data || data?.data?.length === 0) {
    return (
      <div className="mt-8 text-center">
        ...
        <p className="text-gray-600">No reviews available for this provider yet. Be the first to leave a review!</p>
      </div>
    );
  }
