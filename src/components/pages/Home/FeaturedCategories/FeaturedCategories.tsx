...
const FeaturedCategories = () => {
    ...
    return (
      ...
      <FiArrowRightCircle className="ml-2" size={20} />
        ...
        <div className={`grid ${pathname === '/categories' 
    ? 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3' 
    : 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4'} gap-2 md:gap-2 lg:gap-4`}
>
    {isLoading 
        ? pathname === '/categories'
            ? 
              [...Array(3)].map((_, index) => (
                <div 
                    key={index} 
                    className="relative rounded flex items-center justify-center overflow-hidden group bg-gray-200 p-2 "
                    data-aos="fade-up"
                >
                    <div className="relative rounded-lg overflow-hidden border h-[350px] w-full">
                        <div className="block image-wrapper bg-gray-300 h-[250px] w-full"></div>
                        <div className="p-4">
            ...
            : 
              [...Array(4)].map((_, index) => (
                <div 
                    key={index} 
                    className="relative rounded flex flex-col items-center justify-center overflow-hidden bg-gray-200 p-4 "
                    data-aos="fade-up"
                >
                    <div className="mb-4">
                        <span className="inline-block p-5 bg-gray-300 rounded-full h-16 w-16"></span>
                    </div>
                    <div className="h-4 bg-gray-300 rounded w-3/4 mb-2"></div>
                </div>
              ))
        : 
          displayCategories?.map((category: any) => (
            <div 
                key={category.id} 
                className="relative rounded flex items-center justify-center overflow-hidden group bg-white p-2"  
                data-aos="fade-up"
            >
                {pathname === '/categories' ? (
                        ...
                        <div className="relative rounded-lg overflow-hidden border h-[350px]">
                            <div className="block image-wrapper">
                                <img 
                                    ...
                                    className="md:w-[350px] transition-transform duration-300 ease-in-out md:h-[250px]" 
                                />
                            </div>
                            <div className="p-4 flex items-center justify-between">
                                <div className="flex items-center">
                                    <a 
                                        href={`/service-details/${category.id}`} 
                                        className="rounded transition flex flex-col items-center justify-center text-center"
                                    >
                                        <div>
                    ...
                    <Link 
                        ...
                        className="feature-box p-4 rounded transition flex flex-col items-center justify-center text-center z-10"
                    >
                        <div className="mb-4">
                ...
                {pathname !== '/categories' && (
                    <div 
                        className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-0"
                    >
                        <img 
                            src={category.categoryImg} 
                            alt={category.categoryName} 
                            className="w-full h-full object-cover transform translate-x-full group-hover:translate-x-0 transition-transform duration-500" 
                        />
                    </div>
                )}
            </div>
